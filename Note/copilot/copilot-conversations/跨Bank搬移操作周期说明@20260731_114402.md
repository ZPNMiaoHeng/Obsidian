...
[Timestamp: 2026/07/31 11:44:02]

**user**: 如果是跨Bank搬移操作，访问不同bank的读写，可以在1-cycle并行实现：
1、TCAM T2、T3可在1-cycle实现，T4、T5可在1-cycle实现；
2、SRAM T1、T2可在1-cycle实现；
[Context: Notes: Note/Daily/ACL搬移无感实现方案.md]
[Timestamp: 2026/07/31 11:53:19]
...
| 合并项 | 可行性 | 关键依据 |
|:----|:----:|:----|
| TCAM T2+T3 合并 | ✅ 可行 | 原/目不同 bank，写目（bank D）与读原 Mask（bank S）并行无冲突 |
| TCAM T4+T5 合并 | ✅ 可行 | 原/目不同 bank，写目 Mask/vld（bank D）与清原 vld（bank S）并行无冲突 |
| SRAM T1+T2 合并 | ⚠️ **不可行** | T1 读 → T2 写存在 **RAW 数据依赖**，写数据必须等读数据返回 |

### 二、TCAM T2+T3 合并验证（✅）

原 T2 访问**目的 bank**（写 Data + 清目 vld），原 T3 访问**原 bank**（读 Mask）。跨 bank 时两者落在不同 bank，而 TCAM 每个 bank 单访问源、无端口仲裁 [^3]，可同周期并行：

| 合并后 | 目的 bank | 原 bank |
|:----:|:----|:----|
| **T2+T3** | 写 Data(目) + 写 vld(目清0) | 读 Mask(原) |

**依赖检查**：
- T2 写 Data 的数据来自 T1 读（已就绪）✓
- T3 读 Mask 为独立读操作，与 T2 写无数据依赖 ✓

### 三、TCAM T4+T5 合并验证（✅）

原 T4 访问**目的 bank**（写 Mask + 写目 vld），原 T5 访问**原 bank**（清原 vld）。跨 bank 时同样可同周期并行：

| 合并后 | 目的 bank | 原 bank |
|:----:|:----|:----|
| **T4+T5** | 写 Mask(目) + 写 vld(目=src_vld_q) | 写 vld(原清0) |

**依赖检查**：
- T4 写 Mask 的数据来自 T3 读（上一周期，已就绪）✓
- T5 清原 vld 使用 `src_vld_q`（T1 采样，非本周期读）✓

> **额外收益**：T4+T5 合并后，目的 vld 置 1 与原 vld 清 0 在**同一时钟边沿提交**。相比同 bank 场景下 v2.6 需分两拍、存在双 vld 窗口的问题 [^4]，跨 bank 场景**不存在稳定的双 vld 状态**，vld 切换等效于原子操作，安全性反而更高。

### 四、SRAM T1+T2 合并分析（⚠️ 不可行）

SRAM 的 T1（1R 原）与 T2（1W 目）之间存在**读后写（RAW）数据依赖**：

- T2 写入目的地址的数据，正是 T1 从原地址读出的数据
- 即使原/目位于不同 bank（无端口冲突），**写数据也必须等待读数据返回后才能产生**
- 标准同步 SRAM 在 930MHz 下，不可能在同一周期内完成"读→写"的数据传递（除非 SRAM 支持同周期读-写转发特化设计）

**但这不是瓶颈**：跨 bank 合并后 TCAM 需要 3 周期，SRAM 的 2 周期操作（cycle 1 读 + cycle 2 写）完整落在 TCAM 窗口内，**整体 per-entry 周期仍由 TCAM 决定 = 3 cycles**。

### 五、跨 Bank 搬移时序（3 cycles/entry）

| 周期 | TCAM（目的 bank） | TCAM（原 bank） | SRAM |
|:----:|:----|:----|:----|
| **T1** | — | 读 Data(原) + 读 vld(原) | 1R(原) |
| **T2+T3** | 写 Data(目) + 写 vld(目清0) | 读 Mask(原) | 1W(目) |
| **T4+T5** | 写 Mask(目) + 写 vld(目=原) | 写 vld(原清0) | — |

**安全性检查**（与 v2.6.2 对比）：
1. 目的 vld 在 T2+T3 清零，目的 entry 不参与搜索 ✓
2. 原 entry 数据在 T1~T3 期间保持不变，流水线访问原 entry 结果正确 ✓
3. T4+T5 完成 vld 切换：**无双 vld 窗口**（优于同 bank 场景）✓
4. 原 vld=0 的无效 entry 搬移流程不受影响（目的 vld 最终写入 0）✓

**性能收益**：2K entry 全跨 bank 搬移由 2K×5=10K cycles 降至 2K×3=6K cycles（**↓40%**）。

### 六、实现要点

1. **bank 判定**：`cross_bank = (src_addr[bank_bits] != dst_addr[bank_bits])`，每个 entry 独立计算
2. **混合模式**：同一搬移范围内可能交错出现同 bank/跨 bank 条目，状态机需按 per-entry 实时判定选择 5-cycle 或 3-cycle 流程
3. **状态机扩展**：需要新增合并状态，或改用**计数器/微码方式**按模式生成操作序列
4. **IP 能力确认**：跨 bank 并发访问依赖 TCAM IP 支持"同一周期访问两个不同 bank"，建议向 vendor 书面确认（当前已确认的是 vld 与 Data/Mask 并发读写能力 [^5]）

---

是否需要将跨 bank 优化合入 [[ACL搬移无感实现方案]] 并升版为 v2.7？若需要，我可以同步更新时序表、状态机和版本记录。

#### Sources
[^1]: [[跨Bank搬移操作周期说明@20260731_114402]]
[^2]: [[ACL搬移无感实现方案]]
[^3]: [[tcam.smm]]
[^4]: [[TCAM搬移风险评审@20260730_112117]]
[^5]: [[致 TCAM IP Vendor 咨询问题]]
[Timestamp: 2026/07/31 11:56:49]

**user**: 跨 bank 优化合入 ACL搬移无感实现方案 并升版为 v2.7
[Context: Notes: Note/Daily/ACL搬移无感实现方案.md]
[Timestamp: 2026/07/31 13:39:10]

**ai**: 已将跨 Bank 优化完整合入 [[ACL搬移无感实现方案]] 并升版为 **v2.7**，以下为更新后的完整文档：
...
#### 11.2 跨 Bank 并发访问能力（v2.7 待确认）

| 能力项 | 手册支持 | 方案依赖 | 状态 |
|:----|:----:|:----|:----:|
| **同周期访问两个不同 bank**（一个 bank 写、另一个 bank 读） | ❓ | T2+T3 合并：目 bank 写 Data + 原 bank 读 Mask | **待 vendor 书面确认** |
| **同周期访问两个不同 bank**（一个 bank 写、另一个 bank 写） | ❓ | T4+T5 合并：目 bank 写 Mask/vld + 原 bank 清 vld | **待 vendor 书面确认** |

> **风险提示**：跨 bank 并发访问依赖 TCAM IP 支持"同一周期访问两个不同 bank"的能力。当前已确认的是 **vld 与 Data/Mask 并发读写**（同一 bank 内），跨 bank 并发访问需向 vendor 书面确认后方可在验证中解除依赖风险。
...
| **跨 bank 并发访问能力待确认（v2.7）** | **需 vendor 书面确认 TCAM IP 支持同周期访问两个不同 bank；若不支持，跨 bank 条目回退至 5-cycle 同 bank 流程** | **待确认** |
...
- **跨 bank 并发访问能力确认（v2.7）**：与 vendor 确认 TCAM IP 支持同周期访问两个不同 bank 后，补充实测用例

---

: [[ACL搬移无感实现方案]] - 配置约束：不支持执行过程中配置下发，仅支持同时配置，等操作完成后再发起下次
: [[ACL搬移无感实现方案]] - v2.6 优化要点：T0 合入 T2、T1 新增读 vld、T7 合入 T4
: [[ACL搬移无感实现方案]] - v2.6.2 变更：删除 SRAM T5 原地址清 0，vld=0 保证不可命中，残留数据安全
: [[ACL搬移无感实现方案]] - v2.5 关键变更：删除 T5/T6，原 Data/Mask 残留数据无需提前清零
: [[ACL搬移无感实现方案]] - 清除操作，SRAM 无需修改数据，仅操作 TCAM vld
: [[ACL搬移无感实现方案]] - 双 vld 边界场景处理：TCAM IP 固定选择 index 最小条目，两处数据一致
: [[ACL搬移无感实现方案]] - SRAM 操作流程：T1 1R(原)、T2 1W(目)、T5 1W(原清0)
: [[数字电路文档准确性@20260731_085010]] - 修正：IDLE→PRE_CLR_VLD 增加 `!move_en` 互斥，与 7.2 节仲裁优先级一致
: [[搬移地址判断规则@20260730_154855]] - 正序/逆序搬移判断规则
: [[致 TCAM IP Vendor 咨询问题]] - 附录：Valid 位由 `VBE` + `VBI` 独立控制，可伴随 Data/Mask 操作并发进行；支持单独读、单独写
: [[跨Bank搬移操作周期说明@20260731_114402]] - 跨 bank 合并分析：TCAM T2+T3、T4+T5 可合并；SRAM T1+T2 因 RAW 依赖不可合并
...
[Timestamp: 2026/07/31 13:53:33]

**user**: TCAM IP Bank是单端口，对应一组接口；只需要定制多个bank TCAM，内部含有多组接口，就可实现同周期访问两个不同bank
[Context: Notes: Note/Daily/ACL搬移无感实现方案.md]
[Timestamp: 2026/07/31 14:16:09]

**ai**: 这个说明非常关键，**直接消除了 v2.7 方案中最后一个待确认风险**[^1]。

### 风险闭环确认

| 风险项 | 原状态 | 新状态 |
|:----|:----:|:----:|
| **同周期访问两个不同 bank**（一写一读） | 待 vendor 书面确认 | **已确认** ✅ |
| **同周期访问两个不同 bank**（双写） | 待 vendor 书面确认 | **已确认** ✅ |

**原理清晰**：
- 每个 bank 为单端口，对应一组独立接口
- 定制多 bank TCAM 时，内部包含**多组接口**（每个 bank 一组）
- 跨 bank 访问时，T2+T3（目 bank 写 + 原 bank 读）和 T4+T5（目 bank 写 + 原 bank 写）分别落在**不同的接口组**上，天然并行，无端口仲裁冲突[^2]

### 文档更新建议

建议将 [[ACL搬移无感实现方案]] 的 `11.2 跨 Bank 并发访问能力` 标记为**已确认**，并同步更新第十二节风险表：

- 「跨 bank 并发访问能力待确认」→ **已消除**
- 验证建议中「与 vendor 确认 TCAM IP 支持同周期访问两个不同 bank 后，补充实测用例」→ 调整为直接补充实测用例

是否需要我更新文档并升版（v2.7.1），将跨 bank 并发访问能力正式标记为已确认？
...
[^2]: [[跨Bank搬移操作周期说明@20260731_114402]]
[^3]: [[TCAM搬移风险评审@20260730_112117]]
[^4]: [[专家评审实现方案@20260730_112117]]
[^5]: [[搬移地址判断规则@20260730_154855]]
[^6]: [[致 TCAM IP Vendor 咨询问题]]
[^7]: [[数字电路文档准确性@20260731_085010]]
[^8]: [[TCAM_ECC校验实现方案]]
[Timestamp: 2026/07/31 14:17:06]

**user**: 更新文档并升版（v2.7.1）
[Context: Notes: Note/Daily/ACL搬移无感实现方案.md]
[Timestamp: 2026/07/31 14:17:26]

**ai**: ## v2.7.1 变更摘要

本次升版核心变更：**跨 Bank 并发访问能力由"待确认"转为"已确认"**，v2.7 优化前提全部成立，风险表中最后一项待确认风险消除[^1][^2]。

以下为需要更新到 [[ACL搬移无感实现方案]] 的各章节变更内容：
...
### 2. 4.1.2 跨 Bank 场景（补充 IP 结构说明）

在"合并依据"两条之后新增：

> **IP 结构支持**：TCAM IP 每个 bank 为单端口，对应一组独立接口；定制多 bank TCAM 时，内部包含多组接口（每个 bank 一组）。跨 bank 访问时，T2+T3（目 bank 写 + 原 bank 读）与 T4+T5（目 bank 写 + 原 bank 写）分别落在不同 bank 的接口组上，同周期并行无端口仲裁冲突[^1]。

---

### 3. 11.2 跨 Bank 并发访问能力（更新为已确认）
...
> **确认说明**：TCAM IP 的 bank 为单端口结构，每组接口独立服务一个 bank。定制多 bank TCAM 时，内部自然包含多组接口，跨 bank 操作落在不同接口组上，同周期并行无需仲裁[^1]。此确认消除了 v2.7 中最后一项 IP 依赖风险。
...
| 原文 | 更新后 |
|:----|:----|
| 跨 bank 并发访问能力确认（v2.7）：与 vendor 确认 TCAM IP 支持同周期访问两个不同 bank 后，补充实测用例 | **跨 bank 并发访问实测（v2.7.1）**：构造原/目地址跨 bank 场景，验证多接口组同周期独立并行读写无串扰、数据正确 |

---

### 6. 其他部分

- **状态机**：无需变更，`MOVE_MERGED_T2T3` 与 `MOVE_CLR_SRC_VLD` 跨 bank 分支的运行前提已确认
- **性能评估**：数据不变（跨 bank 3 cycles/entry，2K entry 全跨 bank 搬移 ≤ 6.5μs）
- **安全保证分析**：跨 bank vld 原子切换结论不受影响，且实现基础已确认
...
[^1]: [[跨Bank搬移操作周期说明@20260731_114402]]
[^2]: [[ACL搬移无感实现方案]]
[Timestamp: 2026/07/31 14:18:46]

**user**: 直接在笔记中完成这些更新
[Context: Notes: Note/Daily/ACL搬移无感实现方案.md]
[Timestamp: 2026/07/31 14:24:25]

**ai**: 已在 [[ACL搬移无感实现方案]] 中直接完成 v2.7.1 全部更新，以下为更新后的完整文档：
